    ...
    <div className="flex h-screen overflow-hidden">
      <aside className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col">
        ...
        <nav className="flex-1 px-4 space-y-1 mt-4">
          <button type="button" onClick={() => setActiveTab('profile')} className={`flex items-center w-full px-4 py-3 rounded-xl transition-all ${activeTab === 'profile' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/20' : 'text-slate-400 hover:bg-slate-800'}`}>
            <Settings size={18} className="mr-3" /> Perfil
          </button>
          <button type="button" onClick={() => setActiveTab('companies')} className={`flex items-center w-full px-4 py-3 rounded-xl transition-all ${activeTab === 'companies' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/20' : 'text-slate-400 hover:bg-slate-800'}`}>
            <Building2 size={18} className="mr-3" /> Empresas
          </button>
          <button type="button" onClick={() => setActiveTab('trainings')} className={`flex items-center w-full px-4 py-3 rounded-xl transition-all ${activeTab === 'trainings' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/20' : 'text-slate-400 hover:bg-slate-800'}`}>
            <BookOpen size={18} className="mr-3" /> Capacitaciones
          </button>
          <button type="button" onClick={() => setActiveTab('attendance')} className={`flex items-center w-full px-4 py-3 rounded-xl transition-all ${activeTab === 'attendance' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/20' : 'text-slate-400 hover:bg-slate-800'}`}>
            <Users size={18} className="mr-3" /> Asistencias
          </button>
        </nav>

        <div className="p-4 border-t border-slate-800">
          <button type="button" onClick={onLogout} className="flex items-center w-full px-4 py-3 text-red-400 hover:bg-red-500/10 rounded-xl transition-all">
          ...
          {activeTab === 'trainings' && (
            <div className="space-y-8">
              <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl">
                ...
                <input value={trainTitle} onChange={e => setTrainTitle(e.target.value)} className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl mb-4 text-white outline-none focus:ring-2 focus:ring-emerald-500" placeholder="Título del curso (Ej: Prevención de Incendios)" />
                <div className="bg-slate-800/50 p-5 rounded-xl space-y-4 border border-slate-700/50">
                  <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Añadir Módulos de Lectura</p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <input value={newLinkUrl} onChange={e => setNewLinkUrl(e.target.value)} placeholder="Enlace de Drive / URL" className="px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white text-sm outline-none focus:ring-1 focus:ring-emerald-500" />
                  </div>
                  <button type="button" onClick={handleAddLink} className="text-xs text-emerald-500 font-black uppercase tracking-tighter hover:text-emerald-400 flex items-center"><Plus size={14} className="mr-1"/> Añadir Link a la lista</button>
                      ...
                      {state.companies.map(c => {
                        ...
                        return (
                          <div key={c.id} className="bg-slate-950 p-5 rounded-2xl border border-slate-800 flex flex-col items-center hover:border-emerald-500/30 transition-all">
                            ...
                            <div className="bg-white p-3 rounded-xl mb-4 shadow-xl"><QRCodeSVG value={url} size={110} /></div>
                            <button type="button" onClick={() => { navigator.clipboard.writeText(url); alert("Enlace copiado al portapapeles."); }} className="text-xs font-black text-emerald-500 uppercase tracking-widest hover:text-emerald-400">Copiar QR Link</button>
                          </div>
                        );
                      })}
                      {state.companies.length === 0 && <p className="text-xs text-slate-600 italic">Vincule empresas para generar códigos QR.</p>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-6">
              <div className="flex flex-wrap gap-4 items-end bg-slate-900/50 border border-slate-800 p-6 rounded-2xl shadow-xl">
                <div className="flex-1 min-w-[180px]">
                  <label className="text-[10px] font-bold text-slate-500 uppercase mb-1.5 block ml-1">Filtrar por Empresa</label>
                ...
                <div className="flex-1 min-w-[180px]">
                  <label className="text-[10px] font-bold text-slate-500 uppercase mb-1.5 block ml-1">Filtrar por Curso</label>
                ...
                <button type="button" onClick={downloadReport} className="bg-emerald-600 px-8 py-2.5 rounded-xl font-bold text-white flex items-center text-sm hover:bg-emerald-500 transition-all shadow-lg shadow-emerald-900/20"><Download size={18} className="mr-2"/> Exportar Registro PDF</button>
              </div>
              <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl">
                      ...
                      {state.attendances
                        ...
                        .map(a => (
                          <tr key={a.id} className="hover:bg-slate-800/40 transition-colors">
                            ...
                            <td className="px-6 py-4"><span className="px-2 py-1 bg-slate-800 rounded text-xs text-slate-400">{state.companies.find(c => c.id === a.companyId)?.name || 'N/A'}</span></td>
                            <td className="px-6 py-4 text-emerald-500 font-semibold">{state.trainings.find(t => t.id === a.trainingId)?.title || 'N/A'}</td>
                ...
                {state.attendances.length === 0 && (
                  <div className="p-12 text-center text-slate-600 italic font-medium">No se han encontrado registros de asistencia.</div>
                )}
              </div>
            </div>
          )}
